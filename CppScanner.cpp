...
void CppScanner::readNextToken() {
    ...
    read = "";
    if(tryEof()) {
        token = Token("", Token::Type::EOF_IDENT);
        return;
    }
    if(tryStringToken()) //
        return;
...
bool CppScanner::tryAlnum() {
    ...
    try {
        ...
        token = Token(read, type);
    } catch (...){
        token = Token(read, Token::Type::IDENTIFIER);
    }
    return true;
...
bool CppScanner::tryParameterOrAnd() {
    if(in && in.peek() == '&') {
        ...
        if(in && in.peek() == '&') {
            getNextChar();
            token = Token("&&", Token::Type::AND_OPER);
            return true;
        }
        if(!isdigit(in.peek()))
            throwInvalidSyntax();
        ...
        if(std::isalnum(in.peek())|| in.peek() == '\'')
            throwInvalidSyntax();
        token = Token(read, Token::Type::PARAMETER);
        return true;
    }
}
...
bool CppScanner::tryStringToken() {
    if(in && in.peek() == '\'') {
        ...
        if(!in)
            throwNoStringEnding();
        token = Token(read, Token::Type::STRING);
        getNextChar();
        return true;
    }
    return false;
...
bool CppScanner::tryOperator() {
    ...
    switch(c) {
        case '!': getNextChar();
            c = in.peek();
            if(c=='=') { getNextChar(); token = Token("!=", Token::Type::NOT_EQUAL_OPER);}
            else {
                throwInvalidSyntax();
            }
            break;
        case '|': getNextChar();
            c = in.peek();
            if(c=='|') { getNextChar(); token = Token("||", Token::Type::OR_OPER) ;}
            else {
                throwInvalidSyntax();
            }
            break;
        case '=': getNextChar();
            c = in.peek();
            if(c=='=') { getNextChar(); token = Token("==", Token::Type::EQUAL_OPER); }
            else {
                token = Token("=", Token::Type::ASSIGN_OPER) ;
            }
            break;
        case ';': getNextChar(); token = Token(";", Token::Type::SEMICOLON); break;
        case ',': getNextChar(); token = Token(",", Token::Type::COMMA); break;
        case '[': getNextChar(); token = Token("[", Token::Type::SQUARE_BRACKET_LEFT); break;
        case ']': getNextChar(); token = Token("]", Token::Type::SQUARE_BRACKET_RIGHT); break;
        case '(': getNextChar(); token = Token("(", Token::Type::ROUND_BRACKET_LEFT); break;
        case ')': getNextChar(); token = Token(")", Token::Type::ROUND_BRACKET_RIGHT); break;
        case '{': getNextChar(); token = Token("{", Token::Type::CURLY_BRACKET_LEFT); break;
        case '}': getNextChar(); token = Token("}", Token::Type::CURLY_BRACKET_RIGHT); break;
